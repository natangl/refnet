...
// refnet includes.
#include <..\src\script\refnetconsensus.h>
...
int main()
{
    std::cout << "refnetconsensus version: " << refnetconsensus_version() << std::endl;
    ...
    auto op0Result = refnetconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, refnetconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
