 ...
 */
static const char* REFNET_PID_FILENAME = "refnetd.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", REFNET_PID_FILENAME)));
}
 ...
 * between refnetd, and refnet-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", REFNET_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", REFNET_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/refnet/refnet>";
    const std::string URL_WEBSITE = "<https://refnetcore.org>";
...
/** Sanity checks
 *  Ensure that Refnet is running in a usable environment with all
...
{
    // Make sure only a single Refnet process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", REFNET_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if refnet is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if refnet is started while in a temporary directory.\n",
