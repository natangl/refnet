...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " refnet-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  refnet-tx [options] <hex-tx> [commands]  Update hex-encoded refnet transaction\n" +
            "or:     refnet-tx [options] -create [commands]   Create hex-encoded refnet transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded refnet transaction
