#!/usr/bin/env python3
# Copyright (c) 2016-2019 The Refnet Core developers
...
# TestP2PConn: A peer we use to send messages to refnetd, and store responses.
...
class CompactBlocksTest(RefnetTestFramework):
    ...
    # This test actually causes refnetd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that refnetd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for refnetd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # refnetd will not send blocktxn responses for blocks whose height is
