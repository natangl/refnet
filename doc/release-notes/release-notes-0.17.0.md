Refnet Core version 0.17.0 is now available from:

  <https://refnetcore.org/bin/refnet-core-0.17.0/>
...
installer (on Windows) or just copy over `/Applications/Refnet-Qt` (on Mac)
or `refnetd`/`refnet-qt` (on Linux).
...
Refnet Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `refnet.conf` file, not inside included files or from
  ...
  as refnet.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the refnet.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=refnet
    test.uacomment=refnet-testnet
...
be used in V0.17 by starting refnetd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Refnet Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/refnet/bips/blob/master/bip-0174.mediawiki) is an interchange format for Refnet transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://refnettalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Refnet transaction goes through the
following steps:
...
- An **Extractor** produces a valid Refnet transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in RFN. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When refnet is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If refnet is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart refnetd with
...
- When running refnetd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If refnetd
...
- #11962 `0a01843` add seed.refnet.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in RFN (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI refnet:// (krab)
...
- #13528 `b877c39` Move RefnetGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from librefnet-util to librefnet-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for refnet-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling refnet-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use RefnetTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to refnet.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use RefnetTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use RefnetTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use RefnetTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for RefnetTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_refnet` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.refnet is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_refnet` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in refnetd systemd service file (Flowdalic)
- #12652 `55f490a` refnet-cli: Provide a better error message when refnetd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to refnetd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for refnet address in translations (kaplanmaxe)
...
- #13603 `453ae5e` refnet-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix refnet-cli --version (Empact)
...
- #12877 `2b54155` Use refnetd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update refnet-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust refnetcore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed refnet-cli -help output for help2man (hebasto)
