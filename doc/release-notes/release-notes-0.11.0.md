Refnet Core version 0.11.0 is now available from:

  <https://refnet.org/bin/refnet-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer RFN/kB fee to be
...
For example, add the following to `refnet.conf`:
...
related to the blockchain in the refnet system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Refnet Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Refnet Core will 
...
For an example script see [refnet-submittx](https://github.com/laanwj/refnet-submittx).
...
- #5334 `cf87536` librefnetconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new refnet-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only RefnetAmountField
- #5753 `0f44672` Add refnet logo to about screen
...
- #5809 `46bfbe7` Add refnet-cli man page
...
- #5849 `41259ca` contrib/init/refnetd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Refnet-Qt.app to Refnet-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for refnet-tx)
