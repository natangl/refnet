#!/usr/bin/env bash
# Copyright (c) 2016 The Refnet Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   refnetcore.org and refnet.org and compares them.
...
WORKINGDIR="/tmp/refnet_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://refnetcore.org"
HOST2="https://refnet.org"
BASEDIR="/bin/"
VERSIONPREFIX="refnet-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'refnet-',
...
if [ $? -ne 0 ]; then
   echo "refnet.org failed to provide signature file, but refnetcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "refnet.org and refnetcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Refnet Core binary release signing key installed?"
   fi
